...
import { clsx } from "clsx";
import { Collapsible, CollapsibleContent, CollapsibleTrigger } from "@/components/ui/collapsible";
import useOpenSubMenu from "@/hooks/useOpenSubMenu";
import { Separator } from "@/components/ui/separator";
...
interface NavigationGroupProps {
    ...
    openMenuIdx: number;
    handleMenuClick: (menuIdx: number) => void;
}
...
function NavigationGroup({ nav, openMenuIdx, handleMenuClick }: NavigationGroupProps) {
    return (
        <>
            <li>
                <div className="text-xs font-light leading-7 text-gray-500">
...
function NavigationLink({ href, current, icon, name }: NavigationLinkProps) {
    return (
        <SheetClose asChild>
            <Link
                className={clsx(
...
function CollapsibleMenu({ menu, isOpen, onOpenChange }: CollapsibleMenuProps) {
    return (
        <Collapsible open={isOpen} onOpenChange={onOpenChange} className="w-full">
            ...
            <CollapsibleContent className="overflow-hidden data-[state=closed]:animate-collapsible-up data-[state=open]:animate-collapsible-down space-y-1">
                <div className="ml-4 mt-0.5 border-l border-gray-200 p-0">
                    <ul className="space-y-1">
                        {menu.subs.map((sub, idx) => (
                            <li key={idx}>
                ...
                <nav className="flex items-center justify-between px-4 py-3 sm:px-6">
                    <div className="flex items-center">
                        <Sheet>
                            ...
                            <SheetContent side="left" className="flex flex-col">
                                <div className="py-6 sm:mb-6 border-b">
                                    <Link className="flex shrink-0 items-center" href={route('dashboard')}>
                                        <ServerIcon className="mr-2 size-5 shrink-0" />
                                        <span className="text-sm font-semibold">RAPRMDN</span>
                                    </Link>
                                </div>
                                    ...
                                    <ul role="list" className="flex flex-1 flex-col gap-y-1">
                                        {navigations.map((nav, idx) => (
                                            <Fragment key={idx}>
                                                <NavigationGroup
                                                    nav={nav}
                                                    openMenuIdx={openMenuIndex as number}
                                                    handleMenuClick={handleMenuClick}
                                                />
                                            </Fragment>
                                        ))}
                                ...
                                <div className="">
                                    <div className="mb-4 mt-auto w-full rounded-lg border bg-gray-50 shadow-sm">
                                        <a href="#" className="flex items-center gap-x-4 px-3 py-3 text-sm leading-[1.10rem]">
                                            <span className="relative flex shrink-0 overflow-hidden rounded-full size-8">
                                                <img className="aspect-square h-full w-full" src="https://github.com/raprmdn.png" />
                                            </span>
                                            <div>
                                                <span className="sr-only">Your profile</span>
                                                ...
                                                <span aria-hidden="true" className="block text-xs text-gray-500">
                                                    {auth.user.email}
                                                </span>
                                            </div>
                                        </a>
                        ...
                        >
                            <svg xmlns="http://www.w3.org/2000/svg" width={24} height={24} fill="none" viewBox="0 0 24 24" className="size-4">
                                <path
                                    stroke="currentColor"
                                    strokeWidth="1.5"
                                    d="M16 18.25c-.673 1.766-2.21 3-4 3s-3.327-1.234-4-3m8 0h-8m8 0a6.001 6.001 0 0 0-3.197-5.31M8 18.25A6 6 0 0 1 12 8a6 6 0 0 1 3.197 4.94M8 18.25h8m-8 0h8M12 8V3m0 0-3 3m3-3 3 3"
                                    className="transition-colors group-hover:stroke-yellow-500"
                                />
                            </svg>
                        </button>
                        <DropdownMenu>
                            <DropdownMenuTrigger>
                                <Button variant="outline" className="border-0 h-[2.3rem] justify-center gap-2 relative group rounded-full px-4 py-0.5 text-sm">
                                    <span className="whitespace-nowrap">{auth.user.name}</span>
                                    <ChevronDown className="h-4 w-4 group-hover:text-muted-foreground" />
                                </Button>
                            </DropdownMenuTrigger>
                            <DropdownMenuContent className="w-[250px] z-20">
                                <DropdownMenuGroup>
